...
def train_model_b(p, Oi, BATCHSIZE, EPOCH, M, B, input_dim, hidden_dim, output_dim, K_func, C, psi_func, depth, activation = 'tanh', device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
...
if __name__ == '__main__':
    ...
    o_train0 = torch.linspace(int(min(Oi)) - 0.5, int(max(Oi)) + 0.5, B).reshape(B, 1).to(device)
    train_data = Data.TensorDataset(o_train0, torch.zeros(B, 1).to(device))
    train_loader = Data.DataLoader(dataset=train_data, batch_size=BATCHSIZE, shuffle=True)
    ...
    model_b, beta = train_model_b(train_loader, EPOCH, device, model_b, beta, optimizer_b, optimizer_beta)
    pass