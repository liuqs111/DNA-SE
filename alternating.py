...
for item in range(1):
    ...
    for epoch in range(EPOCH):
        for step, (y_train, _) in enumerate(train_loader):
            ...
            # int_denom = torch.zeros(N, 1)
            # int0 = torch.zeros(N, 1)
            # int1 = torch.zeros(N, 1)
            # int_b0 = torch.zeros(N, 1)
            # int_b1 = torch.zeros(N, 1)
            # for i in range(N):
            #     int_denom[i] = 1 - torch.mean(pi_star(rnorm + beta(X[[i]]).data))
            #     int0[i] = torch.mean(rnorm * pi_star(rnorm + beta(X[[i]]).data)) * X[i, 0]
            #     int1[i] = torch.mean(rnorm * pi_star(rnorm + beta(X[[i]]).data)) * X[i, 1]
            #     int_b0[i] = torch.mean(b0(rnorm + beta(X[[i]]).data) * pi_star(rnorm + beta(X[[i]]).data))
            #     int_b1[i] = torch.mean(b1(rnorm + beta(X[[i]]).data) * pi_star(rnorm + beta(X[[i]]).data))
            ...
            # def h(y):
            #     return torch.mean(dnorm(y - beta(X).data))
            # def f0(y):
            #     return torch.mean(int_b0 / int_denom * dnorm(y - beta(X).data))
            # def f1(y):
            #     return torch.mean(int_b1 / int_denom * dnorm(y - beta(X).data))
            # def g0(y):
            #     return torch.mean(
            #         ddnorm(y - beta(X).data) * (-X[:, 0].reshape(N, 1)) + int0 / int_denom * dnorm(y - beta(X).data))
            # def g1(y):
            #     return torch.mean(
            #         ddnorm(y - beta(X).data) * (-X[:, 1].reshape(N, 1)) + int1 / int_denom * dnorm(y - beta(X).data))
            # res0 = sum([(b0(y) * h(y) + f0(y) - g0(y)) ** 2 for y in y_train]) / len(y_train)
            # res1 = sum([(b1(y) * h(y) + f1(y) - g1(y)) ** 2 for y in y_train]) / len(y_train)
            # loss_b = res0 + res1
            ...
            if step % 5 == 0:
                ...
                # int_b1 = torch.zeros(N, 1)
                # for i in range(N):
                #     int_denom[i] = 1 - torch.mean(pi_star(rnorm + beta(X[[i]])))
                #     int0[i] = torch.mean(rnorm * pi_star(rnorm + beta(X[[i]]))) * X[i, 0]
                #     int1[i] = torch.mean(rnorm * pi_star(rnorm + beta(X[[i]]))) * X[i, 1]
                #     int_b0[i] = torch.mean(b0(rnorm + beta(X[[i]])).data * pi_star(rnorm + beta(X[[i]])))
                #     int_b1[i] = torch.mean(b1(rnorm + beta(X[[i]])).data * pi_star(rnorm + beta(X[[i]])))
                ...
                loss_beta.backward()
                optimizer_beta.step()
            # print('Epoch: ', epoch, 'Step:', step, [loss_b.data.numpy(), loss_beta.data.numpy(), beta.state_dict()])
            if step % 25 == 0:
